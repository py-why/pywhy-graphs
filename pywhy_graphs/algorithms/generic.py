...
def is_valid_cg(G: DiUnGraph) -> bool:
    """Check G is a valid Chain Graph (CG).

    A valid CG is one where no undirected edge can be oriented to form
    a directed cycle.

    Parameters
    ----------
    G : DiUnGraph
        The graph with directed and undirected edges.

    Returns
    -------
    bool
        Whether G is a valid CG.
...
def is_valid_cpdag(G: DiUnGraph) -> bool:
    """Check G is a valid CPDAG.

    A valid CPDAG is one where each pair of nodes have
    at most one edge between them and the internal graph of directed edges
    do not form cycles.

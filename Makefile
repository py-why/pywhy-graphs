...
flake:
	@if command -v flake8 > /dev/null; then \
		echo "Running flake8"; \
		flake8 --count pywhy_graphs examples; \
	else \
...
isort:
	@if command -v isort > /dev/null; then \
		echo "Running isort"; \
		isort pywhy_graphs examples doc; \
	else \
...
run-checks:
	isort --check .
	black --check pywhy_graphs examples
	flake8 .
	mypy ./pywhy_graphs
	@$(MAKE) pydocstyle
